...
class TournamentETL:
    ...
    def process_tournament_file(self, file_path: str, challonge_match_id: Optional[int] = None) -> bool:
    ...
    def _load_tournament_data(self, parsed_data: Dict[str, Any]) -> None:
        ...
        for state_data in game_states:
            ...
            if state_data.get('active_player_id') and state_data['active_player_id'] in player_id_mapping:
                state_data['active_player_id'] = player_id_mapping[state_data['active_player_id']]
        ...
        for event_data in events:
            ...
            if event_data.get('player_id') and event_data['player_id'] in player_id_mapping:
        ...
        for territory_data in territories:
            ...
            if territory_data.get('owner_player_id') and territory_data['owner_player_id'] in player_id_mapping:
                territory_data['owner_player_id'] = player_id_mapping[territory_data['owner_player_id']]
        ...
        for resource_data in resources:
            ...
            # Map player_id if present
            if resource_data.get('player_id') and resource_data['player_id'] in player_id_mapping:
                resource_data['player_id'] = player_id_mapping[resource_data['player_id']]
        ...
        for tech_data in technology_progress:
            ...
            # Map player_id if present
            if tech_data.get('player_id') and tech_data['player_id'] in player_id_mapping:
        ...
        if technology_progress:
            ...
            logger.info(f"Inserted {len(technology_progress)} technology progress records")
        ...
        for stat_data in player_statistics:
            ...
            # Map player_id if present
            if stat_data.get('player_id') and stat_data['player_id'] in player_id_mapping:
        ...
        for unit_data in units_produced:
            ...
            # Map player_id if present
            if unit_data.get('player_id') and unit_data['player_id'] in player_id_mapping:
    ...
    def _bulk_insert_game_states(self, game_states: List[Dict[str, Any]]) -> None:
        ...
        with self.db.get_connection() as conn:
            ...
            for state in game_states:
                state_id = conn.execute("SELECT nextval('game_state_id_seq')").fetchone()[0]
                values.append([
                    state_id,
                    state['match_id'],
                    state['turn_number'],
    ...
    def process_directory(self, directory_path: str, file_pattern: str = "*.zip") -> Tuple[int, int]:
        ...
        for file_path in files:
            logger.info(f"Processing file {successful_count + 1}/{len(files)}: {file_path.name}")
        ...
        logger.info(f"Processing complete: {successful_count}/{len(files)} files successful")
    ...
    def validate_data_integrity(self) -> Dict[str, Any]:
        ...
        if orphaned_players and orphaned_players[0] > 0:
            validation_results['errors'].append(f"Found {orphaned_players[0]} orphaned player records")
        ...
        if empty_matches and empty_matches[0] > 0:
            validation_results['warnings'].append(f"Found {empty_matches[0]} matches without players")
        ...
        if invalid_coordinates and invalid_coordinates[0] > 0:
            validation_results['errors'].append(f"Found {invalid_coordinates[0]} territories with invalid coordinates")
...
def process_tournament_directory(directory_path: str, challonge_match_mapping: Optional[Dict[str, int]] = None) -> Dict[str, Any]:
    """Process all tournament files in a directory.
