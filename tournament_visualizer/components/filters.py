...
from ..config import FILTER_OPTIONS, APP_CONSTANTS
...
def create_date_range_filter(
    ...
    default_range: Union[int, str] = 30
) -> dbc.Card:
    ...
    return dbc.Card([
        dbc.CardBody([
            ...
            dcc.Dropdown(
                ...
                options=[{"label": cls, "value": cls} for cls in APP_CONSTANTS["MAP_CLASSES"]],
                className="mb-2"
            )
        ])
    ], className="mb-3")
...
def create_victory_condition_filter(
    ...
    return dbc.Card([
        dbc.CardBody([
            ...
            dcc.Dropdown(
                id=f"{component_id}-dropdown",
                options=[{"label": vc, "value": vc} for vc in APP_CONSTANTS["VICTORY_CONDITIONS"]],
                multi=True,
...
def apply_filters_to_dataframe(
    ...
    if filters.get("players") and "player_name" in filtered_df.columns:
        filtered_df = filtered_df[
            filtered_df["player_name"].isin(filters["players"])
        ]
    ...
    if filters.get("map_sizes") and "map_size" in filtered_df.columns:
        filtered_df = filtered_df[
            filtered_df["map_size"].isin(filters["map_sizes"])
        ]
    
    # Apply map class filter
    if filters.get("map_classes") and "map_class" in filtered_df.columns:
        filtered_df = filtered_df[
            filtered_df["map_class"].isin(filters["map_classes"])
        ]
