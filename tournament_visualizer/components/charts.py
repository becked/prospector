...
from ..config import Config, DEFAULT_CHART_LAYOUT, CIVILIZATION_COLORS
...
def create_match_timeline_chart(df: pd.DataFrame) -> go.Figure:
    ...
    daily_civ_matches = df.groupby(['date', 'winner_civilization']).size().reset_index(name='count')

    # Pivot to get civilizations as columns
    pivot_data = daily_civ_matches.pivot(index='date', columns='winner_civilization', values='count').fillna(0)
    ...
    for i, civ in enumerate(pivot_data.columns):
        color = CIVILIZATION_COLORS.get(civ, Config.PRIMARY_COLORS[i % len(Config.PRIMARY_COLORS)])
...
def create_player_performance_chart(df: pd.DataFrame) -> go.Figure:
    ...
    fig = create_base_figure(
        title="Top 10 Player Performance",
        x_title="Win Rate (%)",
        y_title="Player"
    )
...
def create_civilization_performance_chart(df: pd.DataFrame) -> go.Figure:
    ...
    colors = [CIVILIZATION_COLORS.get(civ, Config.PRIMARY_COLORS[i % len(Config.PRIMARY_COLORS)]) 
              for i, civ in enumerate(df_sorted['civilization'])]
    
    fig = create_base_figure(
        title="Civilization Performance",
        x_title="Win Rate (%)",
        y_title="Civilization"
    )
...
def create_event_timeline_chart(df: pd.DataFrame) -> go.Figure:
    ...
    for event_type in event_types:
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(
                size=8,
                color=Config.PRIMARY_COLORS[y_positions[event_type] % len(Config.PRIMARY_COLORS)]
            ),
...
def create_win_rate_by_map_chart(df: pd.DataFrame) -> go.Figure:
    ...
    fig = create_base_figure(
        title="Average Game Length by Map",
        x_title="Map Type",
...
def create_head_to_head_chart(stats: Dict[str, Any], player1: str, player2: str) -> go.Figure:
    ...
    values = [
        ...
        stats.get('total_matches', 0) - stats.get('player1_wins', 0) - stats.get('player2_wins', 0)
    ]
...
def create_technology_comparison_chart(df: pd.DataFrame) -> go.Figure:
    ...
    player_tech_counts = df.groupby(['player_name', 'civilization'])['count'].sum().reset_index()
...
def create_statistics_grouped_bar(df: pd.DataFrame, category_filter: Optional[str] = None, top_n: int = 10) -> go.Figure:
    ...
    if df.empty:
        return create_base_figure(f"No statistics available for category: {category_filter}")
    ...
    fig = create_base_figure(
        title=f"Top {top_n} Statistics Comparison" + (f" - {category_filter}" if category_filter else ""),
        x_title="Statistic",
        y_title="Value"
    )
...
def create_statistics_radar_chart(df: pd.DataFrame, category_filter: Optional[str] = None, top_n: int = 8) -> go.Figure:
    ...
    for i, player in enumerate(player_names):
        ...
        for stat in top_stats:
            ...
            normalized = (player_val[0] / max_val * 100) if len(player_val) > 0 and max_val > 0 else 0
...
def create_law_milestone_chart(df: pd.DataFrame) -> go.Figure:
    ...
    df_filtered = df[
        df['estimated_turn_to_4_laws'].notna() |
        df['estimated_turn_to_7_laws'].notna()
    ].copy()
...
def create_law_progression_comparison_chart(df: pd.DataFrame) -> go.Figure:
    ...
    if df_filtered.empty:
        return create_empty_chart_placeholder("Not enough data for comparison (need players with 2+ matches)")

    fig = make_subplots(
        rows=1, cols=3,
        subplot_titles=('Average Laws per Game', 'Avg Turn to 4 Laws', 'Avg Turn to 7 Laws'),
    ...
    fig.update_layout(
        title_text="Law Progression Statistics Comparison",
        height=400
    )
...
def create_player_law_performance_chart(df: pd.DataFrame) -> go.Figure:
    ...
    df_filtered = df[
        (df['avg_turn_to_4_laws'].notna()) &
        (df['matches_played'] >= 2)
    ].copy()

    if df_filtered.empty:
        return create_empty_chart_placeholder("Not enough data for performance analysis")
...
def create_map_breakdown_sunburst_chart(df: pd.DataFrame) -> go.Figure:
    ...
    for combination in sorted(combinations):
        ...
        for size in ['Duel', 'Tiny']:
            ...
            size_counts[size] = int(size_data['count'].sum()) if not size_data.empty else 0
