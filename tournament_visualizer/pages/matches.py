...
from tournament_visualizer.components.layouts import (
    ...
    create_empty_state, create_breadcrumb, create_loading_placeholder
)
from tournament_visualizer.components.charts import (
    create_resource_progression_chart, create_territory_control_chart,
    create_event_timeline_chart, create_empty_chart_placeholder,
    create_base_figure, create_technology_comparison_chart,
    create_technology_detail_chart, create_player_statistics_comparison_chart,
    create_statistics_grouped_bar, create_statistics_radar_chart
)
...
layout = html.Div([
    ...
    # Default empty state
    html.Div(id="match-empty-state", children=[
        create_empty_state(
...
def update_match_details(match_id: Optional[int]) -> tuple:
    ...
    try:
        ...
        game_display = players_with_nations if players_with_nations else match_data.get('game_name', 'Unknown')
                            ...
                            dbc.Col([
                                dbc.Card([
                                    dbc.CardHeader([
                                        dbc.Row([
                                            dbc.Col([
                                                html.H5("Top 10 Statistics - Grouped Comparison", className="mb-0")
                                            ], width=8),
                                            dbc.Col([
                                                dcc.Dropdown(
                                    ...
                                    dbc.CardBody([
                                        dcc.Loading(
                                            dcc.Graph(id="match-stats-grouped-bar", config={"displayModeBar": False}),
                                            type="default"
                                        )
                                    ], style={"height": "500px"})
                                ])
                            ...
                            dbc.Col([
                                dbc.Card([
                                    dbc.CardHeader([
                                        dbc.Row([
                                            dbc.Col([
                                                html.H5("Statistics Radar Chart", className="mb-0")
                                            ], width=8),
                                            dbc.Col([
                                                dcc.Dropdown(
                                    ...
                                    dbc.CardBody([
                                        dcc.Loading(
                                            dcc.Graph(id="match-stats-radar", config={"displayModeBar": False}),
                                            type="default"
                                        )
                                    ], style={"height": "500px"})
                                ])
                            ...
                            dbc.Col([
                                dbc.Card([
                                    dbc.CardHeader([
                                        dbc.Row([
                                            dbc.Col([
                                                html.H5("Category Totals", className="mb-0")
                                            ], width=8),
                                            dbc.Col([
                                                dcc.Dropdown(
                                    ...
                                    dbc.CardBody([
                                        dcc.Loading(
                                            dcc.Graph(id="match-stats-comparison", config={"displayModeBar": False}),
                                            type="default"
                                        )
                                    ], style={"height": "500px"})
                                ])
...
@callback(
    Output("match-progression-chart", "figure"),
    Input("match-selector", "value")
)
...
@callback(
    Output("match-breadcrumb", "children"),
    Input("match-selector", "value")
)
    ...
    if match_id:
        ...
        try:
            ...
            if not match_info.empty:
                players_with_nations = match_info.iloc[0].get('players_with_nations', '')
                game_name = match_info.iloc[0].get('game_name', f'Match {match_id}')
                # Use players with nations if available, otherwise use game name
                display_name = players_with_nations if players_with_nations else game_name
...
def update_technology_chart(match_id: Optional[int]):
    ...
    try:
        ...
        if df.empty:
            return create_empty_chart_placeholder("No technology data available for this match")

        return create_technology_comparison_chart(df)

    except Exception as e:
        return create_empty_chart_placeholder(f"Error loading technology data: {str(e)}")


@callback(
    Output("match-technology-detail-chart", "figure"),
    Input("match-selector", "value")
)
def update_technology_detail_chart(match_id: Optional[int]):
    ...
    """
    if not match_id:
        return create_empty_chart_placeholder("Select a match to view technology details")
    ...
    try:
        ...
        return create_empty_chart_placeholder(f"Error loading technology details: {str(e)}")
...
def update_category_filter_options(match_id: Optional[int]) -> tuple[List[Dict[str, str]], List[Dict[str, str]], List[Dict[str, str]]]:
...
def update_stats_grouped_bar(match_id: Optional[int], category_filter: Optional[str]):
    ...
    try:
        ...
        if df.empty:
            return create_empty_chart_placeholder("No statistics data available for this match")

        return create_statistics_grouped_bar(df, category_filter=category_filter, top_n=10)
...
def update_stats_radar(match_id: Optional[int], category_filter: Optional[str]):
    ...
    try:
        ...
        return create_statistics_radar_chart(df, category_filter=category_filter, top_n=8)
...
def update_stats_comparison(match_id: Optional[int], category_filter: Optional[str]):
    ...
    """
    if not match_id:
        return create_empty_chart_placeholder("Select a match to view statistics comparison")
    ...
    try:
        ...
        if category_filter:
            ...
            return create_statistics_grouped_bar(category_df, category_filter=None, top_n=15)
        ...
        category_totals = df.groupby(['stat_category', 'player_name'])['value'].sum().reset_index()
        ...
        return create_empty_chart_placeholder(f"Error loading statistics comparison: {str(e)}")


@callback(
    Output("match-settings-content", "children"),
    Input("match-selector", "value")
)
def update_settings_content(match_id: Optional[int]):
    ...
    """
    if not match_id:
        return create_empty_state("Select a match to view game settings", icon="bi-gear")
    ...
    try:
        ...
        if not metadata:
            return create_empty_state("No settings data available for this match", icon="bi-exclamation-triangle")
        ...
        # Basic settings
        if metadata.get('difficulty') or metadata.get('event_level') or metadata.get('victory_type'):
            basic_settings = dbc.Card([
                dbc.CardHeader("Basic Settings"),
                dbc.CardBody([
                    html.Dl([
                        html.Dt("Difficulty"), html.Dd(metadata.get('difficulty', 'N/A')),
                        html.Dt("Event Level"), html.Dd(metadata.get('event_level', 'N/A')),
                        html.Dt("Victory Type"), html.Dd(metadata.get('victory_type', 'N/A')),
                        html.Dt("Victory Turn"), html.Dd(str(metadata.get('victory_turn', 'N/A')))
                    ])
                ])
            ], className="mb-3")
        ...
        if metadata.get('game_options'):
            try:
                options = json.loads(metadata['game_options']) if isinstance(metadata['game_options'], str) else metadata['game_options']
                if options:
                    game_options_card = dbc.Card([
                        dbc.CardHeader("Game Options"),
                        dbc.CardBody([
                            html.Pre(json.dumps(options, indent=2), className="mb-0")
                        ])
                    ], className="mb-3")
        ...
        if metadata.get('dlc_content'):
            try:
                dlc = json.loads(metadata['dlc_content']) if isinstance(metadata['dlc_content'], str) else metadata['dlc_content']
                if dlc:
                    dlc_card = dbc.Card([
        ...
        if metadata.get('map_settings'):
            try:
                map_settings = json.loads(metadata['map_settings']) if isinstance(metadata['map_settings'], str) else metadata['map_settings']
                if map_settings:
                    map_card = dbc.Card([
                        dbc.CardHeader("Map Settings"),
                        dbc.CardBody([
                            html.Pre(json.dumps(map_settings, indent=2), className="mb-0")
                        ])
                    ], className="mb-3")
        ...
        if not settings_cards:
            return create_empty_state("No detailed settings available", icon="bi-info-circle")

        return html.Div(settings_cards)

    except Exception as e:
        return create_empty_state(f"Error loading settings: {str(e)}", icon="bi-exclamation-triangle")