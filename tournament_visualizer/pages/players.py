...
from tournament_visualizer.components.charts import (
    create_player_performance_chart, create_civilization_performance_chart,
    create_head_to_head_chart, create_empty_chart_placeholder,
    create_base_figure, create_heatmap_chart
)
from tournament_visualizer.components.filters import (
    create_date_range_filter, create_civilization_filter,
...
layout = html.Div([
    ...
    create_filter_card(
        ...
        filters=[
            dbc.Row([
                dbc.Col([
                    create_date_range_filter("players-date", default_range="all")
                ], width=4),
                dbc.Col([
                    create_civilization_filter("players-civilizations")
                ], width=4),
                dbc.Col([
                    html.Label("Minimum Matches:", className="form-label fw-bold"),
                    dcc.Slider(
    ...
    create_tab_layout([
                ...
                dbc.Row([
                    dbc.Col([
                        create_data_table_card(
                            ...
                            columns=[
                                {"name": "Civilization", "id": "civilization"},
                                {"name": "Matches", "id": "total_matches", "type": "numeric"},
                                {"name": "Wins", "id": "wins", "type": "numeric"},
                                {"name": "Win Rate", "id": "win_rate", "type": "numeric", "format": {"specifier": ".1%"}},
                                {"name": "Avg Score", "id": "avg_score", "type": "numeric", "format": {"specifier": ".1f"}},
        ...
        },
        {
            ...
            "content": [
                ...
                dbc.Card([
                    dbc.CardBody([
                        html.H5("Head-to-Head Comparison", className="card-title"),
                        dbc.Row([
                            dbc.Col([
                                html.Label("Player 1:", className="form-label"),
                                dcc.Dropdown(
                            ...
                            dbc.Col([
                                html.Div("vs", className="text-center mt-4"),
                            ], width=2),
                            dbc.Col([
                                html.Label("Player 2:", className="form-label"),
                                dcc.Dropdown(
...
def update_player_summary_metrics(
    ...
    try:
        ...
        most_active = filtered_df.loc[filtered_df['total_matches'].idxmax()] if not filtered_df.empty else None
        top_performer = filtered_df.loc[filtered_df['win_rate'].idxmax()] if not filtered_df.empty else None
        ...
        metrics = [
            ...
            {
                "title": "Most Active",
                "value": most_active['player_name'] if most_active is not None else "N/A",
                "subtitle": f"{most_active['total_matches']} matches" if most_active is not None else "",
                "icon": "bi-activity",
                "color": "success"
            },
            {
                "title": "Top Performer",
                "value": top_performer['player_name'] if top_performer is not None else "N/A",
                "subtitle": f"{top_performer['win_rate']:.1f}% win rate" if top_performer is not None else "",
                "icon": "bi-trophy",
                "color": "warning"
            }
        ]
...
def update_winrate_chart(
    ...
    try:
        ...
        if filtered_df.empty:
            return create_empty_chart_placeholder("No players match the selected criteria")
...
def update_civilization_performance_chart(
    ...
    try:
        ...
        return create_empty_chart_placeholder(f"Error loading civilization data: {str(e)}")
...
@callback(
    ...
    [
        ...
        Input("h2h-player2-selector", "value")
    ]
)
        ...
        results = [
            ...
            create_metric_grid([
                ...
                },
                {
                    ...
                    "value": f"{stats['avg_match_length']:.0f} turns" if stats['avg_match_length'] else "N/A",
                    "icon": "bi-clock",
                    "color": "warning"
                }
            ]),
...
def update_rankings_table(
    ...
    try:
        ...
        filtered_df = filtered_df.sort_values('win_rate', ascending=False).reset_index(drop=True)
...
def update_player_civ_heatmap(
    ...
    try:
        ...
        pivot_data = filtered_df.groupby(['player_name', 'civilization']).agg({
            'wins': 'sum',
            'total_matches': 'sum'
        }).reset_index()

        # Calculate win rate
        pivot_data['win_rate'] = (pivot_data['wins'] / pivot_data['total_matches'] * 100).fillna(0)
...
def update_performance_timeline(
    ...
    try:
        ...
        df = pd.DataFrame(result, columns=['save_date', 'player_name', 'civilization', 'won'])
        ...
        qualified_players = player_match_counts[player_match_counts >= min_matches].index
        ...
        for player in top_players:
            player_data = df_top[df_top['player_name'] == player].sort_values('save_date')
            player_data['cumulative_wins'] = player_data['won'].cumsum()
            player_data['match_number'] = range(1, len(player_data) + 1)
            player_data['cumulative_win_rate'] = (player_data['cumulative_wins'] / player_data['match_number'] * 100)
